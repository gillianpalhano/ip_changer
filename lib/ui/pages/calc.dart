...
class _ConfigNetworkPageState extends State<ConfigNetworkPage> {
  ...
  void _onChanged(String origin) {
    ...
    _debounce = Timer(_delay, () {
      ...
      if (origin == 'ip' && _ipCtrl.text.isNotEmpty) _ipKey.currentState!.validate();
      if (origin == 'mask' && _maskCtrl.text.isNotEmpty) _maskKey.currentState!.validate();
      if (origin == 'gw' && _gwCtrl.text.isNotEmpty) _gwKey.currentState!.validate();
      ...
      if ( mask.isNotEmpty && (ip.isNotEmpty || gw.isNotEmpty) ) {
        if (isMask(mask) && (isIPv4(ip) || isIPv4(gw))) {
          try {
            final netInfo = networkInfo(ip: ip.isNotEmpty ? ip : null, gateway: gw.isNotEmpty ? gw : null, mask: mask);
            setState(() {
      ...
      if (ip.isNotEmpty && mask.isNotEmpty && gw.isNotEmpty && _formKey.currentState!.validate()) {
        try {
    ...
    return Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_netInfoShow) Column(
              children: [
                Text(_netInfoMsg, style: TextStyle(color: _netInfoIsValid ? Colors.green : Colors.red, fontWeight: FontWeight.bold, fontSize: 18)),
                const SizedBox(height: 24),

                if (_netInfo != null) Column(
                  children: [
                    Text('Rede: ${_netInfo!.network}${_netInfo!.maskCidr}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                    Text('Broadcast: ${_netInfo!.broadcast}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                    Text('Faixa de IPs: ${_netInfo!.firstUsable} - ${_netInfo!.lastUsable} (${_netInfo!.totalHosts} IPs)', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                  ],
                )
              ],
            ),
